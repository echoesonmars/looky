...
export function humanizeWardrobeFlowError(raw: string): string {
  ...
  if (/^hf_proxy_|^hf_inference_|^hf_upstream_/i.test(s)) return "Не удалось распознать фото. Попробуйте другое изображение."
  if (/^openai_|^hf_invalid|^hf_token|^unauthorized$/i.test(s)) return "Не удалось обработать. Попробуйте ещё раз."
